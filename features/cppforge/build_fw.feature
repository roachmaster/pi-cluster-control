Feature: CppForge Build Framework Support
  ...
  Scenario: Generate a new executable module
    ...
    And it should contain "CMakeLists.txt", "src/main.cpp", and "include/" folder
  ...
  Scenario: Generate a new library module
    ...
    And it should contain "CMakeLists.txt", "src/utils.cpp", "include/utils.hpp"
  ...
  Scenario: Generate root-level CMakeLists.txt from all modules
    Given a set of modules defined in modules.yaml
    When I run the cppforge bootstrap command
    Then the root CMakeLists.txt should include all module directories
